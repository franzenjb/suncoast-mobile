...
// Enhanced branch data with coordinates
const branchData: Branch[] = [
  ...
  // Add more branches with their coordinates
...
export default function HurricaneMapView() {
  ...
  const stormIcon = L.divIcon({
    html: `<div class="storm-icon">
      <div class="storm-eye animate-spin">
    ...
    <div className="relative h-screen w-full">
      ...
      >
        ...
        {showCone && stormData.uncertaintyCone.map((cone, index) => (
          <Circle
            key={`cone-${index}`}
            center={stormData.forecastTrack.find(p => p.time === cone.time)?.position as [number, number]}
      ...
      <div className="absolute top-4 left-4 bg-white rounded-xl shadow-2xl p-4 z-[1000] max-w-sm">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <p className="text-sm font-semibold text-gray-700">Base Map</p>
            <select
              value={selectedLayer}
            ...
            >
              ...
              <option value="radar">Radar (Premium)</option>
            </select>
          </div>
        </div>
      </div>
